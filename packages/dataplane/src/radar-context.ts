/**
 * Radar evaluation context assembly (moved from apps/api in Phase 4b).
 * One loader, used by the API (baseline at radar creation), the radar worker,
 * and the intelligence plane — they can never disagree about what a condition
 * or an agent sees.
 */
import { computePortfolioSignals, type RadarEvalContextFull } from '@atlas/signal-engine';
import { loadConsolidatedInputs, type Db } from './inputs.js';
...
export function conditionSecurityIds(
  asts: Array<{ metric: { kind: string; securityId?: string } }>,
): string[] {
