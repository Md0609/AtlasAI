...
export interface LlmResponse {
  ...
  degraded: boolean;
  /** Mirrors LlmProvider.generative: false means this text was not model-written. */
  generative: boolean;
}
...
export interface LlmProvider {
  ...
  /**
   * Whether this provider's output is generated by a language model.
   *
   * The fixture returns the deterministic template it was handed, which is a
   * correct answer for a mock and NOT a degradation — but it is also not
   * analysis, and by the time it reaches the user nothing distinguishes the
   * two. `degraded` cannot carry this: it means "the real provider fell back",
   * a different fact. Declared per provider so a future non-generative backend
   * (a replay harness, a demo mode) cannot omit it by accident.
   */
